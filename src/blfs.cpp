...
static int blfs_write(const char *path, const char *buf, size_t size, off_t off, struct fuse_file_info *fi) {
    ...
    if(off + size > i_size){
        ...
        for(int i=cur_last_block + 1;i<=new_last_block;++i){
            inode->add_block(Disk::get_instance()->acquire_unused_block());
        }
